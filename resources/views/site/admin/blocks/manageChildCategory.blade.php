<ul>
    @foreach($childa as $child)
        ...
        >
            ...
            <a class="btn btn-warning btnEditCategory" id="btnEditCategory-{{$child->id}}"
               href="{{ route('category.edit' ,  $child->id) }}"
               data-toggle="tooltip" data-placement="top" title=" {{translate('Edit')}} {{$child->title}}"
            >{{translate('Edit')}}</a>


            @if(count($child->children)>0)
                @include('site.admin.blocks.manageChildCategory',['childa' => $child->children()->orderBy('title', 'ASC')->get()])
            @else
            @endif
        </li>
    @endforeach
